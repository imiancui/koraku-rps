...
### 4.2 時機變拳 (Morph Skill)
- **消耗**：基礎 25 MP（可受 `earring_magatama` 勾玉耳環折抵，最低 5 MP）。
- **機制**：僅在揭曉手勢後的反應窗口內生效，自動轉換為剋制敵方的必勝手勢，重設反應時間為 2.0 秒，並清除敵方獲勝警告 Emoji。
...
### 6.1 蒙眼切西瓜小遊戲
1. 戰勝任一關卡後點擊「請小樂穿泳裝」進入。
2. 固定進行 3 刀，白色指針在時機條中往復擺動。
3. 難度逐刀提升：綠色安全範圍每刀縮小 50%（0.13 -> 0.065 -> 0.0325），擺動速度每刀增加 25%（1800ms -> 1440ms -> 1152ms）。
4. 結算獎勵：每成功切中一刀，在第三刀結算時額外獲得 +100 EXP。
