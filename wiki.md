...
## 35. v0.0.36 版本更新：實體特典小樂抱枕 3D 物理鑑賞商店、預覽圖裁切修正與 WebP 素材全面優化 (v0.0.36)

### 35.1 實體特典・小樂抱枕 3D 互動鑑賞商店 (Special Shop)
- **主選單 09 入口**：主選單新增「09 特典商店」按鈕（深紅底色、金箔字樣、神社視覺），獨立於緣側商店。
- **純原生 WebGL 3D 抱枕引擎 (`Pillow3DViewer.js`)**：
  - 零依賴參數化曲面幾何體演算法 (`buildPillowMesh`，40×112 網格，天然隆起、折痕與縫線)。
  - 四元數旋轉、三層彈簧阻尼懸掛系統（剛度 42/50/13，阻尼 9/12/5.3）。
  - 射線三角形拾取 (`pick`)，支援任意部位抓取、甩動角動量慣性旋轉與重心回正。
  - 01 正面與 02 背面平滑翻轉 180°，背向時自動點亮背面按鈕。支援鍵盤方向鍵微調與 `R` 鍵快速復位。
- **兌換規則與註釋**：標定 `1,000,000` 星砂兌換價，兌換按鈕呈灰色禁用狀態，旁附註釋「尚未開放兌換，敬請期待！」。

### 35.2 預覽圖裁切修復與高畫質鑑賞模式
- **預設視野 Contain 修復**：修復 `.preview-viewport` 內 `<picture>` 缺少約束導致 `img` 依等比放大至 1040px 造成頭耳與腳部裁切之缺陷，修正為預設 100% 完整包含 (contain) 且上下左右安全居中。
- **放大鑑賞切換**：點擊「＋ 放大鑑賞」可展開至 1248px 高清寬度並啟用捲動探索細節；再次點擊恢復完整顯示。
