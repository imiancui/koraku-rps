...
> **當前版本**：`v0.0.27`（顯示於首頁頁腳最左側 `0.0.27`，自最後一位遞增，每 100 個版本進一位：`0.0.100` -> `0.1.0`）  
> **最新更新日期**：2026-09-04  
> **基準規範**：`OPENSPEC.md` 與 `AGENTS.md`  
> **測試狀態**：`npm test` 250/250 全部通過 (100% Pass)；連線版客戶端高精度時間步進器（Ticker）平滑倒數、開拳出拳搖晃與反應時間遞減、存檔紀錄面板「切換回線上模式」URL 參數正規化與模式切換、跨環境自動注入與伺服器雙向切換全數驗證通過。
