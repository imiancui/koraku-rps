...
> **當前版本**：`v0.0.29`（顯示於首頁頁腳最左側 `0.0.29`，自最後一位遞增，每 100 個版本進一位：`0.0.100` -> `0.1.0`）  
> **最新更新日期**：2026-09-04  
> **基準規範**：`OPENSPEC.md` 與 `AGENTS.md`  
> **測試狀態**：`npm test` 253/253 全部通過 (100% Pass)；作弊與調試模組全面重構、後端驗證層扁平與巢狀 stats 相容、一鍵解鎖關卡免污染等級、客戶端深層狀態合併、作弊提交即時重繪視圖、立繪現代 WebP 格式遷移與反作弊 E2E 全鏈路測試全數驗證通過。
