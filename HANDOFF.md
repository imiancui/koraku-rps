...
> **當前版本**：`v0.0.36`（顯示於首頁頁腳最左側 `0.0.36`，自最後一位遞增，每 100 個版本進一位：`0.0.100` -> `0.1.0`）  
> **最新更新日期**：2026-09-05  
> **基準規範**：`OPENSPEC.md` 與 `AGENTS.md`  
> **測試狀態**：`npm test` 264/264、`npm run test:server` 21/21 全部通過；實體特典小樂抱枕 3D 鑑賞（Pillow3DViewer）、預覽圖裁切修正（contain fit）與全面 WebP 化完成。
...
- **Phase 4.19：v0.0.37 升版與發布（已 100% 達成）**：
  - **左上角 Logo BETA 微型標籤**：於全域標頭主標題旁新增神社和風「BETA」微型膠囊標籤（緋紅漸層、細緻金邊、米白字），具備完整 RWD 彈性排版與防溢出保護。
  - **更新日誌 24 小時 GMT+8 時間戳標準化**：更新日誌顯示格式全面升級為標準 24 小時制 `YYYY-MM-DD HH:mm (GMT+8)`，全量回溯補齊 v0.0.0～v0.0.36 所有版本之發布時間戳記，合成兜底函式改用本地 GMT+8 時區計算。
  - **全量測試與資源快取更新**：單元測試 265 項全數通過，更新 `bundle.js` 與靜態資源快取戳記 `?v=202609050850`。
- **Phase 4.20：v0.0.38 升版與發布（已 100% 達成）**：
  - **本地權威伺服器守護整合**：`scripts/serve.mjs` 整合 8080 埠伺服器守護程序，開發與測試時 WebSocket 與 HTTP 雙埠同步在線。
