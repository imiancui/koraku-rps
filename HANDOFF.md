...
> **當前版本**：`v0.0.35`（顯示於首頁頁腳最左側 `0.0.35`，自最後一位遞增，每 100 個版本進一位：`0.0.100` -> `0.1.0`）  
> **最新更新日期**：2026-09-05  
> **基準規範**：`OPENSPEC.md` 與 `AGENTS.md`  
> **測試狀態**：`npm test` 258/258、`npm run test:server` 21/21 全部通過；作弊驗證提示去機敏化（移除 8989 洩露）、遠端權威伺服器 (ws.koraku.app) 同步更新部署並支援 8989 提權。
