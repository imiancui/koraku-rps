...
> **當前版本**：`v0.0.26`（顯示於首頁頁腳最左側 `0.0.26`，自最後一位遞增，每 100 個版本進一位：`0.0.100` -> `0.1.0`）  
> **最新更新日期**：2026-09-04  
> **基準規範**：`OPENSPEC.md` 與 `AGENTS.md`  
> **測試狀態**：`npm test` 247/247 全部通過 (100% Pass)；離線版與連線版雙版本回合倒數、拍點音效、切西瓜跑條正模數防護、三角波計算、技能點消耗與焦點保持、四語系 SP 文案、大劍 burst 雙勝增傷、雙生關卡全員陣亡直接獲勝與 QTE/藥水快捷鍵全數驗證通過。
